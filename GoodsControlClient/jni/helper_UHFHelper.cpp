...
extern "C" {
	...
	JNIEXPORT jint JNICALL Java_helper_UHFHelper_init(JNIEnv *env, jclass clazz) {
		return uhf.init();
	}

	JNIEXPORT jint JNICALL Java_helper_UHFHelper_autoOpenComPort(JNIEnv *env, jclass helper) {
		return uhf.func_AutoOpenComPort(&(uhf.port), &(uhf.ComAdr), uhf.Baud, &(uhf.Frmhandle));
	}
	
	JNIEXPORT jint JNICALL Java_helper_UHFHelper_openComPort(JNIEnv *env, jclass helper, jshort mPort) {
		uhf.port = mPort;
		uhf.Baud = 5;
		uhf.ComAdr = 0xFF;
	...
	JNIEXPORT jint JNICALL Java_helper_UHFHelper_closeComPort(JNIEnv *env, jclass clazz) {
		return uhf.func_CloseComPort();
	}

	JNIEXPORT jint JNICALL Java_helper_UHFHelper_getUHFReaderInfo(JNIEnv *env, jclass helper, jobject uhfReaderInfoObj) {
		return 0;
	}
	JNIEXPORT jint JNICALL Java_helper_UHFHelper_inventory_1G2(JNIEnv *env, jclass helper, jobject beanObj){
	...
	JNIEXPORT jint JNICALL Java_helper_UHFHelper_writeEPC_1G2(JNIEnv *env, jclass helper, jstring jstr_encode) {
		jboolean isCopy;

...
public class GoodsTypeSelectBox extends BorderPane {
	...
	private Label label_current_type;
	/* 当前页码 */
	private int page;
	/* 商品类别列表 */
	...
	public GoodsTypeSelectBox() {
		page = 1;/* 初始化数据 */
		goodsTypeList = FXCollections.observableArrayList();
		URL url = getClass().getResource("goodstype_select.fxml");
	...
	private void initViews(Parent parent) {
		...
		label_current_type = (Label) parent.lookup("#label_current_type");
		
		actionGroup = new ToggleGroup();
		table_goodstype = (TableView<GoodsTypeBean>) parent.lookup("#table_goodstype");

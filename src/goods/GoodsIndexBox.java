...
public class GoodsIndexBox extends BorderPane {
	
	private TextField tf_name;
	private TextField tf_keywords;
	private TextField tf_batch_number;
	private ChoiceBox<String> chb_status;
	private Button btn_search;
	private Button btn_clear;
	private TableView<GoodsBean> table_goods;
	...
	public GoodsIndexBox() {
		page = 1;
		goodsList = FXCollections.observableArrayList();
		URL location = getClass().getResource("goods_index.fxml");
	...
	private void initViews(Parent parent) {
		...
		tf_keywords = (TextField) parent.lookup("#tf_keyword");
		tf_batch_number = (TextField) parent.lookup("#tf_batch_number");
		chb_status = (ChoiceBox<String>) parent.lookup("#chb_status");
	...
	private class ActionTableCell extends TableCell<GoodsBean, String>  {
		...
		protected void editGoods(GoodsBean goodsBean) {
			...
			GoodsEditBox box = new GoodsEditBox(goodsBean);
			...
			dialog.setResultConverter(new Callback<ButtonType, GoodsBean>() {
				...
				public GoodsBean call(ButtonType param) {
					if(param.equals(submitButton)) {
						GoodsBean goodsBean = box.applySubmit();
